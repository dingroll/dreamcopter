...
function dreamPlanner(desc) {
  ...
  function getSlackUsername(userId) {
    ...
    var userData = slackMaps.users.get(userId);
  ...
  function slackMessageToDingroll(body) {
    return body.replace(/<([^>]+)>/g, function(match, inside) {
      ...
      if (/^#/.test(inside)) {
        ...
        var channel = slackMaps.channels.get(inside.slice(1));
        if (channel) {
          return '#' + channel.name;
        } else {
  ...
  function addStarterDingrollMessages(dayDoc) {
    var channel = dayDoc._id.slice(0,dayDoc._id.indexOf('/'));
    var firstGroup = priorityGroups(channel)[0];
    if (firstGroup) {
      var tags = tagsForChannelGroup(channel, firstGroup);
      for (var i = 0; i < dayDoc.messages.length; i++) {
        // TODO: handle skipping over messages that have been deleted
        var message = dayDoc.messages[i];
        if (!message.dingrollMessages) {
          message.dingrollMessages = !message.slackMessage.subtype ? [{
            group: firstGroup,
            tags: tags,
            filterLength: tags.length,
            body: slackMessageToDingroll(message.slackMessage.text)
          }] : [];
        }
        // TODO: Don't clobber this?
        message.username = getSlackUsername(message.slackMessage.user);
      }
    }
    return dayDoc;
  }
  ...
  return {
    loadSlackMaps: loadSlackMaps,
    addStarterDingrollMessages: addStarterDingrollMessages,
